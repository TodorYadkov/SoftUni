...
const getAllReceips = async () => {
    ...
    const responseAllReceips = await fetch('http://localhost:3030/data/recipes?select=_id%2Cname%2Cimg%2C_ownerId');
...
const getReceipById = async (id) => {
    // Get the promise and transform into response using await
    const responseReceip = await fetch(`http://localhost:3030/data/recipes/${id}`);
    // Check status code on received response
...
const checkIsLogged = () => {
    if (sessionStorage.getItem('authenticationToken') !== null) {
...
const logout = async () => {
    try {
        const response = await fetch('http://localhost:3030/users/logout', {
            method: 'get',
            headers: {
                'X-Authorization': sessionStorage.getItem('authenticationToken')
            },
        });
        // https://github.com/softuni-practice-server/softuni-practice-server#usage
